...
function amenityIcon(id) {
  ...
  return `<svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="#FF385C" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="${d}"/></svg>`;
}
      ...
      <div class="listing-images" data-id="${l.id}">
        ...
        <button class="listing-wishlist${inWL?' active':''}" onclick="toggleWishlist(event,${l.id})" aria-label="Save venue">
          <svg viewBox="0 0 32 32" width="20" height="20"
               fill="${inWL?'#FF385C':'none'}" stroke="${inWL?'none':'white'}" stroke-width="2">
...
function toggleWishlist(e, id) {
  ...
  svg.setAttribute('fill',   saved ? '#FF385C' : 'none');
  ...
  if (cardBtn && cardBtn !== btn) {
    ...
    csvg.setAttribute('fill',   saved ? '#FF385C' : 'none');
  ...
  if (modalBtn && modalBtn !== btn && appState.selectedListing?.id === id) {
    ...
    msv.setAttribute('fill',   saved ? '#FF385C' : 'none');
...
function openListing(id) {
  ...
  setTimeout(() => {
    ...
    const dot = L.divIcon({
      className: '',
      html: '<div style="width:14px;height:14px;border-radius:50%;background:#FF385C;border:3px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,0.35)"></div>',
  ...
  document.getElementById('listingModalHighlights').innerHTML = highlightItems.map(h => `
    <div class="highlight-item">
      ${h.id ? amenityIcon(h.id) : '<svg viewBox="0 0 24 24" width="24" height="24" fill="#FF385C"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 14.5v-9l6 4.5-6 4.5z"/></svg>'}
    ...
    <div class="amenities-list">
      ${venueAmenities.map(a => `
        <div class="amenity-row">
          <svg viewBox="0 0 20 20" width="18" height="18" fill="#FF385C" style="flex-shrink:0;margin-top:2px"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
          <div>
  ...
  if (modalBtn) {
    ...
    modalSvg.setAttribute('fill',   inWL ? '#FF385C' : 'none');
