  ...
  try {
    ...
    queue.forEach(n => {
      ...
      RESERVATIONS.push({
        ...
        guest:       n.guest,
        bandName:    n.bandName || '',
        guestImg:    n.guestImg || 'https://api.dicebear.com/7.x/initials/svg?seed=Artist&backgroundColor=FF385C&textColor=ffffff',
        property:    n.property,
...
function populateTopbarAvatar() {
  ...
  wrap.innerHTML = `<img src="${user.avatar}" alt="${user.firstName}"
    onerror="this.style.background='#FF385C'" style="width:100%;height:100%;object-fit:cover"/>`;
}
...
function renderEarningsChart(canvasId) {
  ...
  months.forEach((m,i) => {
    ...
    bw.innerHTML = `
      <div class="chart-bar" style="height:${pct}%;background:${future?'rgba(255,255,255,0.05)':'rgba(255,56,92,0.25)'}"
    ...
    bw.querySelector('.chart-bar').addEventListener('mouseenter', function(){
      if(!future) this.style.background='#FF385C';
    });
    bw.querySelector('.chart-bar').addEventListener('mouseleave', function(){
      if(!future) this.style.background='rgba(255,56,92,0.25)';
    });
    wrap.appendChild(bw);
  });
  ...
  <div class="section">
    ...
    <table class="fin-table">
      ...
      <tbody>
        ...
        <tr><td>Remaining balance</td><td>$${remaining.toLocaleString()}</td><td>Settled per venue contract terms</td></tr>
      ...
      ${filtered.map(a => {
        ...
        <label style="display:flex;align-items:center;gap:10px;font-size:14px;color:var(--text-sec);cursor:pointer;padding:5px 0;transition:color .15s">
          <input type="checkbox" value="${a.id}" ${checked ? 'checked' : ''}
                 style="accent-color:#FF385C;width:16px;height:16px"
...
function openNewListingModal() {
  ...
  if (inner) inner.innerHTML = `
    ...
    <p>Drag &amp; drop photos here, or <span style="color:#FF385C;font-weight:600">browse</span></p>
...
function renderImgPreview() {
  ...
  if (nlmImages.length > 0 && inner) {
    inner.innerHTML = `<svg viewBox="0 0 24 24" width="32" height="32" fill="var(--text-muted)"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
      <p><span style="color:#FF385C;font-weight:600">Add more photos</span> &nbsp;(${nlmImages.length}/20)</p>`;
  }
}
...
function renderAnalytics() {
  ...
  const stats = [
    { label: 'Total requests', value: total,      color: '#FF385C' },
  ...
  months.forEach((m, i) => {
    ...
    bw.className = 'chart-bar-wrap';
    bw.innerHTML = `
      <div class="chart-bar" style="height:${pct}%;background:rgba(255,56,92,0.25)" title="${counts[i]} requests"></div>
      <div class="chart-bar-label">${m.label}</div>`;
    bw.querySelector('.chart-bar').addEventListener('mouseenter', function(){ this.style.background='#FF385C'; });
    bw.querySelector('.chart-bar').addEventListener('mouseleave', function(){ this.style.background='rgba(255,56,92,0.25)'; });
    wrap.appendChild(bw);
  });
  ...
  document.getElementById('analyticsGenres').innerHTML = sorted.map(([type, count]) => `
    <div style="margin-bottom:10px">
      ...
      <div style="height:6px;background:var(--border);border-radius:3px;overflow:hidden">
        <div style="height:100%;width:${Math.round((count/genreMax)*100)}%;background:#FF385C;border-radius:3px;transition:width 0.4s"></div>
      </div>
...
function renderEarnings() {
  ...
  document.getElementById('earningsStatsGrid').innerHTML = [
    { icon:'💰', label:'Total earned (all time)', value:`$${totalAllTime.toLocaleString()}`,  delta:'GigNVenue + self-managed', deltaClass:'delta-up',      color:'#FF385C' },
