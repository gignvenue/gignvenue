...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const _firstLogin = !user.firstName && !user.artistName;
    ...
    if (uuidReqIds.length > 0) {
      ...
      (msgRows || []).forEach(row => {
        ...
        MY_MESSAGES.unshift({
          ...
          unread:   _isThreadUnread(row.booking_id, row.created_at, row.sender_id, row.sender_type),
          thread:   [],
        });
      });
    }
  ...
  try {
    const { data: preMsgRows } = await gnvClient
      .from('messages')
      .select('venue_id, body, created_at, sender_id, sender_type')
      .eq('artist_id', user.id)
      .is('booking_id', null)
      .order('created_at', { ascending: false });
    ...
    if (profileSection && !document.getElementById('firstLoginBanner')) {
      ...
      banner.innerHTML = `
        <div style="flex:1">
          ...
          <div style="color:#aaa;font-size:13px;line-height:1.5">Fill in your artist profile so venues know who they're booking. The more detail you provide, the better your chances of getting booked.</div>
        </div>
...
function _isThreadUnread(threadId, lastMsgCreatedAt, lastSenderId, lastSenderType) {
  if (lastSenderType === 'artist') return false;
  try {
    const reads = JSON.parse(localStorage.getItem(`gnv_read_${user.id}`) || '{}');
