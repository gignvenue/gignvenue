...
async function onTourMapClick(e) {
  ...
  loadEl.style.cssText = 'width:22px;height:22px;border-radius:50%;background:rgba(255,45,120,0.35);border:2px dashed #FF2D78;box-sizing:border-box';
  ...
  pinEl.style.cssText = 'background:#FF2D78;color:#fff;font-size:12px;font-weight:700;width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;border:2px solid #fff;box-shadow:0 2px 8px rgba(0,0,0,0.6);opacity:0.55';
      ...
      <div style="min-width:215px;padding:2px 0">
        ...
        <div style="display:flex;gap:8px">
          <button onclick="confirmMapStop()"
            style="flex:1;background:#FF2D78;color:#fff;border:none;border-radius:6px;padding:8px;font-size:13px;font-weight:600;cursor:pointer">
            Add stop
          </button>
...
function confirmMapStop() {
  ...
  if (!date) { if (dateEl) dateEl.style.border = '1.5px solid #FF2D78'; return; }
  ...
  if (geocoded.length > 1) {
    if (routeGeometry) {
      const line = L.geoJSON(routeGeometry, {
        style: { color: '#FF2D78', weight: 3, opacity: 0.85 }
      }).addTo(_tourMap);
